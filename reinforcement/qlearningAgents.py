...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.qvalues[state][action] if state in self.qvalues else 0.0
    ...
    def computeValueFromQValues(self, state):
        ...
        for acao in self.getLegalActions(state):
            values.append(self.getQValue(state, acao))
    ...
    def computeActionFromQValues(self, state):
        ...
        bestActions = []
        for acao in self.getLegalActions(state):
            if self.getQValue(state, acao) == bestQValue:
                bestActions.append(acao)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        features = self.featExtractor.getFeatures(state, action)
        for key in features:
            self.weights[key] += self.alpha * (reward + bestQValue - QValue) * features[key]
